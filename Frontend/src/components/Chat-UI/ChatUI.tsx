...
export default function ChatUI({ backend }: { backend: ChatBackend }) {
  ...
  const [minVoicePopup, setMinVoicePopup] = useState(false);

  // Sync speechStatus with SpeechSynthesis events
  useEffect(() => {
    const synth = window.speechSynthesis;
    function handleStart() { setSpeechStatus('playing'); }
    function handlePause() { setSpeechStatus('paused'); }
    function handleResume() { setSpeechStatus('playing'); }
    function handleEnd() { 
      setSpeechStatus('stopped');
      synth.cancel(); // Ensure speech is fully stopped
    }
    function handleError() { setSpeechStatus('stopped');
      synth.cancel();
     }
  ...
  return (
    <div className="h-screen overflow-hidden" style={{ background: "linear-gradient(180deg,#021827 0%, #001420 100%)" }}>
            ...
            <div className="space-y-4">
              ...
              <div className="px-2 space-y-3">
                ...
                {messages.length > 0 && messages[messages.length - 1].role === "assistant" && (
                  <div className="flex gap-3 justify-end mt-2 mb-2">
                    {/* Pause/Resume button */}
                    <button
                      title={speechStatus === 'paused' ? "Resume voice reply" : "Pause voice reply"}
                      onClick={() => {
                      ...
                      className="w-10 h-10 flex items-center justify-center rounded-full bg-white shadow hover:bg-yellow-50"
                      style={{ zIndex: 10 }}
                    >
                      {speechStatus === 'paused' ? (
                        // Play icon
                    ...
                    {/* Stop/Play button */}
                    <button
                      title={speechStatus === 'playing' || speechStatus === 'paused' ? "Stop voice reply" : "Play voice reply from start"}
                      onClick={() => {
                        if (speechStatus === 'playing' || speechStatus === 'paused') {
                          window.speechSynthesis.cancel();
                          setSpeechStatus('stopped');
                        } else if (speechStatus === 'stopped') {
                          setShowVoicePopup(true); // Ensure popup reappears
                      ...
                      className="w-10 h-10 flex items-center justify-center rounded-full bg-white shadow hover:bg-red-50"
                      style={{ zIndex: 10 }}
                    >
      ...
      {/* Side Voice Reply Popup */}
      {messages.length > 0 && messages[messages.length - 1].role === "assistant" && showVoicePopup && (
        <div
          className={`fixed right-0 top-1/4 bg-white shadow-2xl rounded-l-2xl px-6 py-5 flex flex-col items-center gap-4 z-[1201] animate-slideInVoice ${minVoicePopup ? 'w-16 px-2 py-2' : 'w-[260px]'}`}
          style={minVoicePopup ? { width: 34 } : { width: 260 }}
        >
          <div className="flex w-full justify-between items-center mb-2">
            <span className="font-semibold text-gray-800">Voice Reply</span>
            <div className="flex gap-2">
              {/* Minimize button */}
              <button
                title="Minimize"
                onClick={() => setMinVoicePopup(true)}
              ...
              <button
                title="Close"
                onClick={() => { window.speechSynthesis.cancel(); setSpeechStatus('stopped'); setShowVoicePopup(false); setMinVoicePopup(false); }}
                className="w-7 h-7 flex items-center justify-center rounded-full bg-gray-100 hover:bg-gray-200 text-gray-500"
              >
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4">
            ...
            <>
              <div className="flex gap-4">
                ...
                <button
                  ...
                  className={`w-10 h-10 flex items-center justify-center rounded-full bg-yellow-50 shadow hover:bg-yellow-100 ${speechStatus === 'playing' ? 'animate-pulse' : ''}`}
                >
                  {speechStatus === 'paused' ? (
                    // Play icon
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 text-green-500">
                ...
                <button
                  ...
                  className={`w-10 h-10 flex items-center justify-center rounded-full bg-red-50 shadow hover:bg-red-100 ${speechStatus === 'playing' ? 'animate-pulse' : ''}`}
                >
                  {speechStatus === 'playing' || speechStatus === 'paused' ? (
                    // Stop icon
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 text-red-500">
              ...
              {speechStatus === 'playing' && (
                <div className="mt-3 flex gap-1">
                  {[...Array(5)].map((_, i) => (
                    <div key={i} className="w-2 h-6 rounded bg-yellow-300 animate-voiceBar" style={{ animationDelay: `${i * 0.15}s` }} />
                  ))}
                </div>
              )}
          ...
          {minVoicePopup && (
            <button
              title="Restore"
              onClick={() => setMinVoicePopup(false)}
              className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-100 hover:bg-gray-200 text-gray-500 mt-2"
            >
      ...
      {/* Voice bar animation CSS */}
      <style>{`
      @keyframes slideInVoice {
        from { transform: translateX(100%); opacity: 0; }
        to   { transform: translateX(0); opacity: 1; }
      }
      .animate-slideInVoice {
        animation: slideInVoice 0.4s cubic-bezier(.2,.9,.2,1);
      }
      @keyframes voiceBar {
        0%, 100% { height: 8px; }
        50% { height: 24px; }
    ...
    {recording && (
      <div
        className="fixed right-0 top-1/4 bg-white shadow-2xl rounded-l-2xl px-6 py-5 flex flex-col items-center gap-4 z-[1202] animate-slideInVoice w-[260px]"
        style={{ width: 260 }}
      >
        <div className="flex w-full justify-between items-center mb-2">
        ...
        <div className="text-gray-500 text-xs mt-2">Speak your message. When done, click stop or pause.</div>
      </div>
    )}
    </div>
  );
